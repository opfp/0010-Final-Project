...
def main( argc, argv ):
	...
	while(True):
		...
		while(True):
			...
			ppf_out_list.append(shape_cords[:brack2].replace(" ", "") + "\n")
	...
	ppf_out = open("pgh.ppf", "w")
	ppf_out.writelines(ppf_out_list)
	ppf_out.close()
...
# 	datapath += os.path.sep + "data"
#
# def load_polygons_pef(path):
# 	raw_plygons = open(datapath + os.path.sep + path, "r").read()
# 	raw_plygons = raw_plygons[raw_plygons.index("name")+4:]
# 	raw_plygons = raw_plygons[raw_plygons.index("name")+4:]
